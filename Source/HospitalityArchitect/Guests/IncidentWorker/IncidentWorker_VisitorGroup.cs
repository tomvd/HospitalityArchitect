...
namespace HospitalityArchitect
{
    ...
    public class IncidentWorker_VisitorGroup : IncidentWorker_NeutralGroup
    {
        ...
        public override void ResolveParmsPoints(IncidentParms parms)
        {
            if (parms.points < 0f)
            {
                parms.points = Rand.ByCurve(pointsCurve); // TODO depend on guest type
            }
        }
        ...
        {
            var totalAmount = parms.pawnCount;
        ...
        protected virtual IEnumerable<Pawn> GenerateNewPawns(IncidentParms parms, int preferredAmount)
        {
            ...
            {
                var newPawns = PawnGroupMakerUtility
                    .GeneratePawns(IncidentParmsUtility.GetDefaultPawnGroupMakerParms(PawnGroupKindDef, parms, true),
                        false).ToArray();
        ...
        private static void GiveItems(IEnumerable<Pawn> visitors)
        {
            foreach (var visitor in visitors)
            {
                ...
                var amountS = Mathf.RoundToInt(((wealthBase + (visitor.ageTracker.AgeBiologicalYears / 5.0f)))); // eldery can get better beds to compensate for their lower mood
                if (amountS > 0)
                {
        ...
        public static void CreateLord(Faction faction, IntVec3 chillSpot, List<Pawn> pawns, Map map, bool showLetter, bool getUpsetWhenLost, int duration)
        {
            ...
            var lordJob = new Hospitality.LordJob_VisitColony(faction, chillSpot, duration, getUpsetWhenLost);
            ...
            if (showLetter)
            {
                ...
                Messages.Message( pawns[0].kindDef.LabelCap+ " arrived", MessageTypeDefOf.PositiveEvent);
            }
