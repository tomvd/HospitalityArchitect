...
using Verse;
using JobDriver_StandBy = HospitalityArchitect.Reception.JobDriver_StandBy;
...
namespace HospitalityArchitect.Reception
{
	 ...
	 */
    public class ReceptionController : IExposable
    {
		...
		private void UpdateCaches(bool odd)
		{
			if (odd)
			{
				...
				WorkingPawns.AddRange(pawns.Where(pawn =>
					...
					|| pawn.jobs?.curDriver is JobDriver_Host sellJob && sellJob.job.targetA.Thing == Register));
			}
			else
			{
...
//					.Where(t => StorefrontUtility.IsBuyableAtAll(null, t, this)).Take(50).ToList());
        ...
        public ReceptionController(Map map)
        {
            Map = map;
        }
