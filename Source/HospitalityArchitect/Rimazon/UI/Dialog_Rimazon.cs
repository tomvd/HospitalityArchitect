...
{
    public class Dialog_Rimazon : MainTabWindow
    {
        ...
        private void DoThingRow(Thing thing, Rect rect)
        {
            ...
            {
                _rimazonService.CurrentOrder.AddToCart(thing, false);
            }

            columnRect.x += 20f;
            if (Widgets.ButtonText(columnRect, ">>"))
            {
                _rimazonService.CurrentOrder.AddToCart(thing, true);
            }
        }
        ...
        private void FillInventory()
        {
            ...
            foreach (var thingDef in DefDatabase<ThingDef>.AllDefs.Where(def => def.category == ThingCategory.Building))
            {
                if (thingDef.GetStatValueAbstract(StatDefOf.MarketValue) > 0)
                {
                    ...
                    if (!(category.ToLower().Contains("null") || category.ToLower().Contains("special") || category.ToLower().Contains("ship") || category.ToLower().Contains("structure") || category.ToLower().Contains("corps")))
                    {
                        if (!inventory.ContainsKey(category))
                        {
                            inventory.Add(category, new List<Thing>());
